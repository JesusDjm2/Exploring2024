...
@section('titulo', 'Crear tour en portugués')
...
@section('contenido')
    ...
    <div class="row">
        <div class="col-lg-12">
            <h3 class="float-left">Crear Nuevo Tour en Portugués</h3>
            <a href="{{ route('tourspt.index') }}" class="btn btn-primary float-right">Volver</a>
        </div>
        <div class="col-12 mt-2">
            <form action="{{route('tourspt.store')}}" method="post" enctype="multipart/form-data" class="bg-light" style="padding: 1em">
                @csrf
                <div class="row">
                    <div class="col-lg-6">
                    ...
                    <div class="col-lg-6 pt-3">
                        <label for="categoria" class="form-label">Categoría: <small>Es posible escoger más de 1 Cat. presionando CTRL</small></label>
                        <select multiple="multiple" name="categoria[]" id="categoria" class="form-control form-control-sm">
                            <option value="caminata" {{ in_array('caminata', old('categoria', [])) ? 'selected' : '' }}>Diarios</option>
                            <option value="machuPicchu" {{ in_array('machuPicchu', old('categoria', [])) ? 'selected' : '' }}>Paquetes Cusco</option>
                            <option value="around" {{ in_array('around', old('categoria', [])) ? 'selected' : '' }}>Trek & Aventura</option>
                            <option value="luxury" {{ in_array('luxury', old('categoria', [])) ? 'selected' : '' }}>Paquetes en Perú</option>
                        </select>
                        @error('categoria')
                            <div class="alert alert-danger mt-1">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-lg-12 pt-3">
                        <label for="keywords" class="form-label">Keywords: <small>(Separar cada palabra/frase por una coma)</small></label>
                        <input type="text" id="keywords" name="keywords" class="form-control form-control-sm" required value="{{ old('keywords') }}">
                        @error('keywords')
                            <div class="alert alert-danger mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-lg-12 pt-3">
                        ...
                        <input type="text" id="slug" name="slug" class="form-control form-control-sm" onkeydown="handleSpace(event)" required value="{{ old('slug') }}">
                        @error('slug')
                            <div class="alert alert-danger mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <a href="{{ route('tourspt.index') }}" class="btn btn-secondary mt-4">Cancelar</a>
