    ...
    <div class="box-body">
        ...
        <div class="form-group">
            ...
            <input type="text" name="descripcion" value="{{ old('descripcion', $blog->descripcion) }}"
                ...
                placeholder="Descripcion" maxlength="105">
        ...
        <div class="form-group">
            ...
            <textarea name="cuerpo" class="ckeditor form-control form-control-sm {{ $errors->has('cuerpo') ? 'is-invalid' : '' }}"
        ...
        <div class="form-group">
            ...
            <input type="file" name="img"
                class="form-control form-control-sm {{ $errors->has('img') ? 'is-invalid' : '' }}">
            @if ($blog->img)
                <img src="{{ asset($blog->img) }}" alt="Imagen del blog" style="width: 400px">
            @endif
            @if ($errors->has('img'))
                <div class="invalid-feedback">
                    {{ $errors->first('img') }}
        ...
        <div class="form-group">
            <label for="blog_id">Relacionar con Blog</label>
            <select name="blog_id" id="blog_id" class="form-control form-control-sm @error('blog_id') is-invalid @enderror">
                <option value="">Seleccione un blog</option>
                @foreach ($relaciones as $id => $nombre)
                    <option value="{{ $id }}" {{ old('blog_id') == $id ? 'selected' : '' }}>
                        {{ $nombre }}
                    </option>
                @endforeach
            </select>
