<div class="box box-info padding-1">
    <div class="box-body">
        ...
        <div class="form-group">
            <label for="relacion">Relacionar con Blog</label>
            <select name="blog_id" id="relacion" class="form-control @error('blog_id') is-invalid @enderror">
                <option value="">Selecciona un blog</option>
                @foreach($relaciones as $id => $nombre)
                    <option value="{{ $id }}" {{ (old('blog_id') == $id || isset($blog) && $blog->blog_id == $id) ? 'selected' : '' }}>{{ $nombre }}</option>
                @endforeach
            </select>
            @error('blog_id')
            ...
            @foreach ($tags as $id => $tag)
                <div class="form-check form-check-inline">
                    <input type="checkbox" name="tags[]" value="{{ $id }}" class="form-check-input"
                        {{ in_array($id, old('tags', $blog->entags->pluck('id')->toArray())) ? 'checked' : '' }}>
